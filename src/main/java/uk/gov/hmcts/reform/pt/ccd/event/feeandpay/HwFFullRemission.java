...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.HWF_FULL_REMISSION;

@Component
@RequiredArgsConstructor
public class HwFFullRemission implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(HWF_FULL_REMISSION.getId(), this::submit)
            .forState(State.PENDING_CASE_ISSUED)
            .name(HWF_FULL_REMISSION.getName())
            .description(HWF_FULL_REMISSION.getName())
