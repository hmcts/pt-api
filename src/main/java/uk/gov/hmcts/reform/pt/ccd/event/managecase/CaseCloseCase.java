...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_CLOSE_CASE;

@Component
@RequiredArgsConstructor
public class CaseCloseCase implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_CLOSE_CASE.getId(), this::submit)
            .forStates(
                State.CASE_ISSUED,
            ...
            .name(CASE_CLOSE_CASE.getName())
            .description(CASE_CLOSE_CASE.getName())
    ...
    private SubmitResponse<State> submit(EventPayload<PTCase, State> payload) {
        return SubmitResponse.<State>builder().state(State.CLOSED).build();
    }
}
