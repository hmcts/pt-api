...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.STAYS_CREATE_STAY;

@Component
@RequiredArgsConstructor
public class StaysCreateStay implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(STAYS_CREATE_STAY.getId(), this::submit)
            .forStateTransition(CASE_PROGRESSION, CASE_STAYED)
            .name(STAYS_CREATE_STAY.getName())
            .description(STAYS_CREATE_STAY.getName())
