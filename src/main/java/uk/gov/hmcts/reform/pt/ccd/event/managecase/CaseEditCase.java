...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_EDIT_CASE;

@Component
@RequiredArgsConstructor
public class CaseEditCase implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_EDIT_CASE.getId(), this::submit)
            .forStates(
                State.CASE_ISSUED,
                State.CASE_PROGRESSION
            )
            .name(CASE_EDIT_CASE.getName())
            .description(CASE_EDIT_CASE.getName())
