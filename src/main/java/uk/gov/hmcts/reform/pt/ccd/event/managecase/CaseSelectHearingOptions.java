...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_SELECT_HEARING_OPTIONS;

@Component
@RequiredArgsConstructor
public class CaseSelectHearingOptions implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_SELECT_HEARING_OPTIONS.getId(), this::submit)
            .forStates(
                State.CASE_ISSUED,
                State.CASE_PROGRESSION,
                State.HEARING_READINESS
            )
            .name(CASE_SELECT_HEARING_OPTIONS.getName())
            .description(CASE_SELECT_HEARING_OPTIONS.getName())
