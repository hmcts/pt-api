...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_CONTACT_PARTIES;

@Component
@RequiredArgsConstructor
public class CaseContactParties implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_CONTACT_PARTIES.getId(), this::submit)
            .forStates(CASE_ISSUED, CASE_PROGRESSION, HEARING_READINESS)
            .showSummary()
            .name(CASE_CONTACT_PARTIES.getName())
            .description(CASE_CONTACT_PARTIES.getName())
