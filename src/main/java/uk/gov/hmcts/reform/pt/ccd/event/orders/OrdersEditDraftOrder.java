...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.ORDERS_EDIT_DRAFT_ORDER;

@Component
@RequiredArgsConstructor
public class OrdersEditDraftOrder implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(ORDERS_EDIT_DRAFT_ORDER.getId(), this::submit)
            .forStates(CASE_ISSUED, CASE_PROGRESSION, HEARING_READINESS)
            .showSummary()
            .name(ORDERS_EDIT_DRAFT_ORDER.getName())
            .description(ORDERS_EDIT_DRAFT_ORDER.getName())
