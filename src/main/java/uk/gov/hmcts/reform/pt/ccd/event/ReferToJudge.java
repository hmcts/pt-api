...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.REFER_TO_JUDGE;

@Component
@RequiredArgsConstructor
public class ReferToJudge implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(REFER_TO_JUDGE.getId(), this::submit)
            .forStates(
                CASE_ISSUED,
                CASE_PROGRESSION,
                HEARING_READINESS,
