package uk.gov.hmcts.reform.pt.ccd.event.documentmanagement;
...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.DOCUMENT_MANAGEMENT_AMEND;
...
public class DocumentManagementAmend implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(DOCUMENT_MANAGEMENT_AMEND.getId(), this::submit)
            .forStates(
                CASE_ISSUED,
                CASE_PROGRESSION,
                HEARING_READINESS,
