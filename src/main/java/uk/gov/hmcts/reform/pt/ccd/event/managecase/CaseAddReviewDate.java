...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_ADD_REVIEW_DATE;

@Component
@RequiredArgsConstructor
public class CaseAddReviewDate implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_ADD_REVIEW_DATE.getId(), this::submit)
            .forStates(
                State.CASE_ISSUED,
            ...
            .name(CASE_ADD_REVIEW_DATE.getName())
            .description(CASE_ADD_REVIEW_DATE.getName())
