...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.HWF_NO_REMISSION;

@Component
@RequiredArgsConstructor
public class HwFNoRemission implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(HWF_NO_REMISSION.getId(), this::submit)
            .forState(State.PENDING_CASE_ISSUED)
            .name(HWF_NO_REMISSION.getName())
            .description(HWF_NO_REMISSION.getName())
