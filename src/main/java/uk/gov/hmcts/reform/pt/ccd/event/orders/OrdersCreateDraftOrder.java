package uk.gov.hmcts.reform.pt.ccd.event.orders;
...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.ORDERS_CREATE_DRAFT_ORDER;

@Component
@RequiredArgsConstructor
public class OrdersCreateDraftOrder implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(ORDERS_CREATE_DRAFT_ORDER.getId(), this::submit)
            .forStates(CASE_ISSUED, CASE_PROGRESSION, HEARING_READINESS)
            .name(ORDERS_CREATE_DRAFT_ORDER.getName())
            .description(ORDERS_CREATE_DRAFT_ORDER.getName())
