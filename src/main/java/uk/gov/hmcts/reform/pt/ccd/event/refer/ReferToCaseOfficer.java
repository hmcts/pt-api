...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.REFER_TO_CASE_OFFICER;

@Component
@RequiredArgsConstructor
public class ReferToCaseOfficer implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(REFER_TO_CASE_OFFICER.getId(), this::submit)
            .forStates(
                CASE_ISSUED,
                CASE_PROGRESSION,
                HEARING_READINESS,
