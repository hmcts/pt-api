...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_SUBMIT_FURTHER_INFO;

@Component
@RequiredArgsConstructor
public class CaseSubmitFurtherInfo implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_SUBMIT_FURTHER_INFO.getId(), this::submit)
            .forStates(CASE_ISSUED, CASE_PROGRESSION, HEARING_READINESS)
            .name(CASE_SUBMIT_FURTHER_INFO.getName())
            .description(CASE_SUBMIT_FURTHER_INFO.getName())
