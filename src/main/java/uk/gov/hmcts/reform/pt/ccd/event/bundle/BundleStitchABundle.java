package uk.gov.hmcts.reform.pt.ccd.event.bundle;
...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.BUNDLE_STITCH_A_BUNDLE;

@Component
@RequiredArgsConstructor
public class BundleStitchABundle implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(BUNDLE_STITCH_A_BUNDLE.getId(), this::submit)
            .forState(PREPARE_FOR_HEARING_CONDUCT_HEARING)
            .showSummary()
            .name(BUNDLE_STITCH_A_BUNDLE.getName())
            .description(BUNDLE_STITCH_A_BUNDLE.getName())
