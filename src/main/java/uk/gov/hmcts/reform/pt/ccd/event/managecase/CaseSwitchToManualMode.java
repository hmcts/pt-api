...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.CASE_SWITCH_TO_MANUAL_MODE;

@Component
@RequiredArgsConstructor
public class CaseSwitchToManualMode implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(CASE_SWITCH_TO_MANUAL_MODE.getId(), this::submit)
            .forStates(
                State.CASE_ISSUED,
            ...
            .name(CASE_SWITCH_TO_MANUAL_MODE.getName())
            .description(CASE_SWITCH_TO_MANUAL_MODE.getName())
