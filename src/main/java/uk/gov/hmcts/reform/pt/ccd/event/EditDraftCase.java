...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.EDIT_DRAFT_CASE;

@Component
@RequiredArgsConstructor
public class EditDraftCase implements CCDConfig<PTCase, State, UserRole> {
    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(EDIT_DRAFT_CASE.getId(), this::submit)
            .forStates(AWAITING_SUBMISSION_TO_HMCTS, PENDING_CASE_ISSUED)
            .showSummary()
            .name(EDIT_DRAFT_CASE.getName())
