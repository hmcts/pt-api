...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.FEE_PAYMENT_SUCCESSFUL;

@Component
@RequiredArgsConstructor
public class FeePaymentSuccess implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(FEE_PAYMENT_SUCCESSFUL.getId(), this::submit)
            .forState(State.PENDING_CASE_ISSUED)
            .name(FEE_PAYMENT_SUCCESSFUL.getName())
            .description(FEE_PAYMENT_SUCCESSFUL.getName())
