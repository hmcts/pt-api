...
import static uk.gov.hmcts.reform.pt.ccd.event.EventId.HWF_MORE_INFORMATION_REQUIRED;

@Component
@RequiredArgsConstructor
public class HwFMoreInformationRequired implements CCDConfig<PTCase, State, UserRole> {

    @Override
    public void configureDecentralised(DecentralisedConfigBuilder<PTCase, State, UserRole> configBuilder) {
        configBuilder
            .decentralisedEvent(HWF_MORE_INFORMATION_REQUIRED.getId(), this::submit)
            .forState(State.PENDING_CASE_ISSUED)
            .name(HWF_MORE_INFORMATION_REQUIRED.getName())
            .description(HWF_MORE_INFORMATION_REQUIRED.getName())
